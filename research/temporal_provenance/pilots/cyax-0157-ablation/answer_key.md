...
## K1 — Governing Issue #157

Issue #157 ("Remove current machine-local path disclosures") is OPEN. Its
Project status is Verification. It was created 2026-09-11T02:06:26Z and last
updated 2026-09-11T20:50:02Z. The Issue requests removal of confirmed
personal and machine-local filesystem disclosures from public repository
surfaces while preserving scientific identity, numerical behavior, and
reproducibility.

**Source:** `github:Julia-meets-String-Theory/CYAxiverse.jl#157` body at
SHA-256 `409b589c...`

## K2 — Supporting PR #158
...
Verification. PR #158 state is also OPEN with Project status Verification.
Neither has been closed or moved to a different column.

**Source:** GitHub API `state` and `projectItems[].status` fields for both
#157 and #158 as recorded in `source_snapshot.json`.

## K4 — PR #158 exact metadata

PR #158 is OPEN, non-draft, unmerged (`mergedAt: null`), and mergeable. Its
head commit is `8f6a9c28ac4b778b07f244dbbbc0076c86dc1a45` with tree
...
**Source:** GitHub API fields for PR #158 as recorded in
...
(R-001 through R-005) covering GitHub prose sanitization, runtime path
configuration, scientific behavior preservation, operational compatibility,
and live-reference closure.
...
## K6 — CI boundary and focused test evidence
...
**Source:** PR #158 `statusCheckRollup` in `source_snapshot.json`; PR #158
body verification section.

## K7 — Independent re-review evidence and its limitations
...
`reviews[]` array for PR #158 is empty. No formal GitHub review decision
(APPROVED, CHANGES_REQUESTED, or COMMENTED) is exposed through the API at
...
**Source:** PR #158 body (self-reported review); PR #158 `reviews: []` in
`source_snapshot.json`.

## K8 — R-005 remains open
...
**Source:** Issue #157 body; PR #158 body; `specs/0157-public-path-remediation/spec.md` R-005 section.

## K9 — PR #158 must not close Issue #157
...
**Source:** PR #158 comments in `source_snapshot.json`; PR #158
`headRefOid`.

## K11 — Evidence limitations and abstentions

The following evidence limitations apply at the observation time:

1. **No independent CI rerun:** The scorer/curator has not independently
   rerun CI. The CI results are from the GitHub Actions surface as recorded.
2. **Timestamp-scoped GitHub surface:** All GitHub observations are scoped
   to the observation time (approximately 2026-09-13T01:22Z). State may
   change after the snapshot.
3. **Self-reported verification:** The PR body verification section
...
4. **Empty reviews array:** GitHub exposes no formal review decision,
   leaving the review status as a self-reported claim.
5. **No production execution:** No production CYTools geometry generation,
   database scan, retired workflow, or external evidence generation was
   independently verified.
6. **Branch surface not scanned:** R-005 branch scan has not been
...
and the explicit non-execution statements in the PR body.

## K12 — Next valid action

The next valid action for this work item is:

1. **Obtain and record an exact-head review and owner merge decision for
...
2. **If PR #158 is merged, keep Issue #157 open.** Merge incorporates the
   current-surface remediation but does not complete R-005.
3. **Separately perform R-005:** Execute the surviving public branch
