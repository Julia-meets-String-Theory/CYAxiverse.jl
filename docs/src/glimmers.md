...
This page documents the small Glimmers pilot implemented in
`CYAxiverse.glimmers`. It is designed to run against the complete geometry
files already present in
`/Users/vmehta/Documents/CYAxiverse/cyaxiverse/data`.

The formulas are the core EFT results of *Glimmers from the Axiverse*
(arXiv:2309.13145v3), applied to the local CYAxiverse file convention.

The pilot implements the paper's core hierarchy and photon-coupling routines:
...
This is an adapted local pilot, not a reproduction of the paper's 200,000-model
...
package's `Matrix{Int}` convention in reusable 4096-column chunks. The pilot
...
julia --project=. scripts/glimmers_local_pilot.jl \
  --data-dir /Users/vmehta/Documents/CYAxiverse/cyaxiverse/data \
  --h11 15,100,200,300 \
  --limit 2 \
  --output /private/tmp/cyaxiverse-glimmers-pilot.csv
```
...
the EM charge. If it is omitted, the pilot uses the direct divisor with the
smallest stored `prime_divisor_volumes` value. That default is a reproducible
technical choice, not an identification of the Standard Model divisor or the
QCD divisor.
...
```julia
using CYAxiverse
const G = CYAxiverse.glimmers

results = G.run_local_pilot(
...
`load_potential(path)` reads the package-shaped datasets without reorienting
them:
...
`L[2, i] = log10 scale`. The pilot retains the sign from the first row and
...
`load_geometry(path)` requires the following fields:
...
For this pilot, each column of `effective_cone` is treated as a direct divisor
...
pilot; it is not asserted to be the paper's exact SM/QCD divisor construction.
...
This is the paper-compatible guard for the selected leading terms. The local
...
signs remain in `GlimmersPotential.coefficient_signs`.

## Scope boundary

The current pilot is intended to answer bounded numerical questions such as:
...
- exact reconstruction of the paper's divisor/SM/QCD sampling procedure;
...
Use the standalone handoff
`/Users/vmehta/Documents/CYAxiverse/cyaxiverse/HANDOFF_GLIMMERS_LOCAL_PILOT.md`
for the reproducibility contract and the next-agent checklist.
