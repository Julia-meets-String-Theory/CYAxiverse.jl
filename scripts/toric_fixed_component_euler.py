...
def transverse_component_euler_orbit(certificate):
    ...
    for cone_rays in _all_fan_cones(certificate["quotient_maximal_cones"]):
        ...
        if orbit_dimension == 0 or affine_rank == 0:
            ...
            independent = SympyMatrix(differences.tolist()).columnspace()
            difference_basis = np.column_stack([
                np.asarray(vector, dtype=int).reshape(-1) for vector in independent
            ])
            if not general_l._sublattice_is_saturated(difference_basis):
                raise ValueError(
                    "lower-dimensional Newton-face difference lattice is not saturated"
                )
