...
The merged and gap-analysis paths must each have an explicit SHA-256 entry;
the replay driver freezes both digests and rechecks the merged digest directly
before decoding it. Current h11=4/5 artifacts contain no schema-valid MPCP
certificates, so full execution is blocked. Use
`--allow-terminal-only-smoke --max-rows 2` only for infrastructure plumbing;
such output is labelled `infrastructure_smoke_only` and
`no_scientific_result` and performs zero database writes.
