...
def load_mpcp_certificates(path):
    ...
    if not isinstance(certificates, list) or not certificates:
        raise RuntimeError(
            "bounded MPCP certificate input must contain a non-empty certificates list"
        )
    ...
    for position, certificate in enumerate(certificates):
        ...
        if key in seen:
            raise RuntimeError(
                f"duplicate bounded MPCP certificate identity: {key[0]}::{key[1]}"
            )
        seen.add(key)
    ...
    return {
        ...
        "identity_digest": hashlib.sha256(
            json.dumps(
                sorted(f"{polytope}::{frst}" for polytope, frst in seen),
                separators=(",", ":"),
            ).encode("utf-8")
        ).hexdigest(),
...
def main():
    ...
    population_preflight = (
        run_population_preflight(package_root, args.h11, Path(args.parquet_dir))
        if args.h11 in (4, 5) else None
    )
...
def _build_one_class(args, np_index, record, ledger_zst_path, ledger_sha256,
    ...
    for qcd_divisor_index in range(n_prime_divisors):
        ...
        try:
            ...
            # Publish with an atomic create-only link.  The earlier existence
            # check is advisory: another bridge process can publish the same
            # key while this class is being built, so a final os.replace would
            # silently clobber its artifact.
