...
def _dual_vertex_parity_evidence(matrix, torus_shift, lambda_f, dual_vertices):
    ...
    for vertex in np.asarray(dual_vertices, dtype=int):
        if np.array_equal(matrix.T @ vertex, vertex):
            ...
            # lambda_f=1, so it must pass this check.
            if parity == 1:
...
def classify_smoothness(
    ...
    parity = _dual_vertex_parity_evidence(
        matrix, torus_shift, lambda_f, dual_vertices
    )
    ...
    for component in fixed_components:
        ...
        if component["fixed_toric_dimension"] == 3:
            ...
            if n_s is None:
                ...
                non_smooth_reasons.append("eq. (4.50) gives n_S = 0")
